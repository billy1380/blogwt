...
package com.willshex.blogwt.client.animation.gdx.math;
...
public abstract class Interpolation {
	...
	static public class PowOut extends Pow {
		...
		public float apply (float a) {
			return (float) Math.pow(a - 1, power) * (power % 2 == 0 ? -1 : 1)
					+ 1;
		}
	}
	...
	static public class Exp extends Interpolation {
		...
		public float apply (float a) {
			if (a <= 0.5f)
				return ((float) Math.pow(value, power * (a * 2 - 1)) - min)
						* scale / 2;
			return (2 - ((float) Math.pow(value, -power * (a * 2 - 1)) - min)
					* scale) / 2;
		}
	};
	...
	static public class BounceOut extends Interpolation {
		...
		public BounceOut (int bounces) {
			if (bounces < 2 || bounces > 5) throw new IllegalArgumentException(
